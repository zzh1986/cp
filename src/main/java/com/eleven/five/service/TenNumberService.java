...
import com.eleven.five.mapper.TenNumberMapper;
...
public class TenNumberService {
     ...
     */
    public void insertIntoTenNumbers() {
        tenNumberMapper.deleteAll();
        //需要进行双层的循环插入
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 75; j++) {
                ...
                TenNumber tenNumber = new TenNumber();
                tenNumber.setId(null);
                tenNumber.setOneNum(oneNum);
                tenNumber.setTwoNum(twoNum);
                tenNumber.setThreeNum(threeNum);
                tenNumber.setFourNum(fourNum);
                tenNumber.setFiveNum(fiveNum);
                tenNumber.setSixNum(sixNum);
                tenNumber.setSevenNum(sevenNum);
                tenNumber.setEightNum(eightNum);
                tenNumber.setNineNum(nineNum);
                tenNumber.setTenNum(tenNum);
                tenNumber.setElevenNum(elevenNum);
                //处理排序问题
...
//                Arrays.sort(sort);
                ...
                tenNumber.setPeriod(period);
                tenNumberMapper.save(tenNumber);
            }
        }
    }
